...
func (gc *GarbageCollector) sweep(ctx job.Context) error {
	...
	*blobCnt, *mfCnt, *sweepSize = 0, 0, 0
	runner1 := utils.NewLimitedConcurrentRunner(BatchSize)
	for _, blob := range gc.deleteSet {
		// copy !
		curBlob := blob
		runner1.AddTask(func() error {
			...
			count, err := gc.blobMgr.UpdateBlobStatus(ctx.SystemContext(), curBlob)
			if err != nil {
			...
			if _, exist := gc.trashedArts[curBlob.Digest]; exist && curBlob.IsManifest() {
				runner2 := utils.NewLimitedConcurrentRunner(BatchSize)
				for _, art := range gc.trashedArts[curBlob.Digest] {
					// copy !
					curArt := art
					runner2.AddTask(func() error {
						...
						gc.logger.Infof("delete the manifest with registry v2 API: %s, %s, %s",
							curArt.RepositoryName, curBlob.ContentType, curBlob.Digest)
						if err := v2DeleteManifest(gc.logger, curArt.RepositoryName, curBlob.Digest); err != nil {
							gc.logger.Errorf("failed to delete manifest with v2 API, %s, %s, %v", curArt.RepositoryName, curBlob.Digest, err)
							if err := ignoreNotFound(func() error {
								return gc.markDeleteFailed(ctx, curBlob)
							}); err != nil {
						...
						if err := retry.Retry(func() error {
							return ignoreNotFound(func() error {
								return gc.registryCtlClient.DeleteManifest(curArt.RepositoryName, curBlob.Digest)
							})
						}, retry.Callback(func(err error, sleep time.Duration) {
							gc.logger.Infof("failed to exec DeleteManifest, error: %v, will retry again after: %s", err, sleep)
						})); err != nil {
							gc.logger.Errorf("failed to remove manifest from storage: %s, %s, errMsg=%v", curArt.RepositoryName, curBlob.Digest, err)
							if err := ignoreNotFound(func() error {
								return gc.markDeleteFailed(ctx, curBlob)
							}); err != nil {
								gc.logger.Errorf("failed to call gc.markDeleteFailed() after gc.registryCtlClient.DeleteManifest() error out: %s, %s, %v", curArt.RepositoryName, curBlob.Digest, err)
								runner2.Cancel(err)
								return err
							}
							skippedBlob = true
						...
						gc.logger.Infof("delete artifact trash record from database: %d, %s, %s", curArt.ID, curArt.RepositoryName, curArt.Digest)
						if err := ignoreNotFound(func() error {
							return gc.artrashMgr.Delete(ctx.SystemContext(), curArt.ID)
						}); err != nil {
							gc.logger.Errorf("failed to call gc.artrashMgr.Delete(): %v, errMsg=%v", curArt.ID, err)
							runner2.Cancel(err)
							return err
						}
						return nil
			...
			if err := ignoreNotFound(func() error {
				return gc.blobMgr.Delete(ctx.SystemContext(), curBlob.ID)
			}); err != nil {
